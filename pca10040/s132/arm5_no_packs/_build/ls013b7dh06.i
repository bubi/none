...
#line 4 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
#line 5 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
...
#line 6 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
#line 7 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
...
#line 8 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
...
#line 9 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
...
#line 10 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
#line 1 "..\\..\\..\\..\\none\\font.h"
...
#line 11 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"
#line 12 "..\\..\\..\\..\\..\\..\\components\\drivers_ext\\LS013B7DH06\\LS013B7DH06.c"

static app_timer_t vcom_timer_data = { {0} }; static const app_timer_id_t vcom_timer = &vcom_timer_data;


static const nrf_drv_spi_t spi = { . p_registers = (0 == 1 ? (void *)((NRF_SPIM_Type *) 0x40003000UL) : (void *)((NRF_SPI_Type *) 0x40003000UL)), . irq = SPIM0_SPIS0_TWIM0_TWIS0_SPI0_TWI0_IRQn, . drv_inst_idx = 0, . use_easy_dma = 0 };
display_status m_disp_state;

uint8_t  m_tx_buf[48+3];
uint8_t  m_rx_buf[48+3];
uint8_t  m_length;
uint8_t  m_line_number;
uint8_t  m_line_number_end;
uint8_t* m_fb;
...
void ls013_spi_event_handler(nrf_drv_spi_evt_t const * p_event)
{
		switch(m_disp_state)
		{	
			case SEND_LINE:
				m_tx_buf[0]=0;
				for(uint8_t i=0;i<8;i++) m_tx_buf[0] |= (m_line_number & 0x80>>i) ? 1<<i : 0 ;
				...
				if(m_line_number == m_line_number_end){
					m_tx_buf[48 + 2] = 0x00;
					m_line_number = 0;
					m_disp_state = SEND_END;
					nrf_drv_spi_transfer(&spi, m_tx_buf, (48 + 2), m_rx_buf, 0);	
				}
...
static void ls013_spi_init(void)
{
    nrf_drv_spi_config_t spi_config = { . sck_pin = 0xFF, . mosi_pin = 0xFF, . miso_pin = 0xFF, . ss_pin = 0xFF, . irq_priority = 6, . orc = 0xFF, . frequency = NRF_DRV_SPI_FREQ_4M, . mode = NRF_DRV_SPI_MODE_0, . bit_order = NRF_DRV_SPI_BIT_ORDER_MSB_FIRST, };
    ...
    do { const uint32_t LOCAL_ERR_CODE = (nrf_drv_spi_init(&spi, &spi_config, ls013_spi_event_handler)); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler_bare((LOCAL_ERR_CODE)); } while (0); } } while (0);
}


void vcom_timer_evt(void * p_context)
{
...
ret_code_t ls013_vcom_init(void)
{
		uint32_t err_code;
		
		nrf_drv_gpiote_init();
		nrf_drv_gpiote_out_config_t config = { . init_state = 1 ? NRF_GPIOTE_INITIAL_VALUE_HIGH : NRF_GPIOTE_INITIAL_VALUE_LOW, . task_pin = 0, };
		err_code = nrf_drv_gpiote_out_init(22, &config);
		...
		err_code = app_timer_create(&vcom_timer, APP_TIMER_MODE_REPEATED,vcom_timer_evt);
		do { const uint32_t LOCAL_ERR_CODE = (err_code); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler_bare((LOCAL_ERR_CODE)); } while (0); } } while (0);
		app_timer_start(vcom_timer, ((uint32_t)(((((1000/60)/2) * (uint64_t)32768) + ((((0) + 1) * 1000) / 2)) / (((0) + 1) * 1000))), 0); 
...
uint8_t* ls013_init(void)
{
	...
	nrf_drv_gpiote_out_config_t config = { . init_state = 1 ? NRF_GPIOTE_INITIAL_VALUE_HIGH : NRF_GPIOTE_INITIAL_VALUE_LOW, . task_pin = 0, };
	nrf_drv_gpiote_out_init(24, &config);
	nrf_drv_gpiote_out_clear(24);
	
	memset(fb,0x00,48*128);
...
ret_code_t  ls013_showframe(uint8_t* fb){
		...
		m_tx_buf[0] = 0x80;
		m_line_number = 0x01;
		m_line_number_end = 128;
		m_disp_state = SEND_LINE;
		
		 
		nrf_drv_gpiote_out_set(24);
		err = nrf_drv_spi_transfer(&spi, m_tx_buf, 1, m_rx_buf, 0);
		return err;
}
...
ret_code_t ls013_clearframe(void){
		...
		nrf_drv_gpiote_out_set(24);
		err = nrf_drv_spi_transfer(&spi, m_tx_buf, 2, m_rx_buf, 0);
	   
	  while(m_disp_state != WAITING);
		return err;
}
...
ret_code_t  printText(uint8_t* fb, uint8_t x, uint8_t y, char* text, uint8_t length, uint8_t color){
	...
	while(length){
		c =*text;
		for(i=0;i<8;i++){
			line_bits = font8x8_basic[c][i];
			for(a=0;a<8;a++){
					bit = (line_bits >> (a)) & 0x01;
					if(bit){
						ls013_drawPixel(fb,(x_off+a),(y+i),color);	
					}
			}
		}	
		text++;
...
ret_code_t ls013_drawPixel(uint8_t* fb, uint8_t x, uint8_t y, uint8_t color){
	...
	if(x>128 || y>128) return ((0x0) + 0);
	
	if(bit_pos <= 5){
		*draw = *draw & ~(mask << (shift - bit_pos));
		*draw = *draw | (color << (shift-bit_pos)); 
	}else{
		overrun = shift-bit_pos;
		signed_overrun = 4294967295u - overrun + 1;
		*draw  = *draw & ~(mask >> signed_overrun);
		*draw  = *draw | (color >> signed_overrun);
		draw++;
		*draw  = *draw & ~(mask << (8-signed_overrun)); 
		*draw  = *draw | (color << (8-signed_overrun));
	}
	return ((0x0) + 0);
}
...
ret_code_t fillWindow(uint8_t* fb, uint8_t x, uint8_t y,uint8_t x_end,uint8_t y_end, uint8_t color){
			
	for(int i=y;i<y_end;i++){
		for(int a=x;a<x_end;a++){
				ls013_drawPixel(fb, a, i, color);
		}
	}
	return ((0x0) + 0);
}

display_status ls013_getStatus(){
	return m_disp_state;
}

void ls013PackPix(uint8_t* buf,uint8_t cx,uint8_t x,uint8_t y,uint8_t c){
